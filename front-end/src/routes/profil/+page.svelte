<script>
    // Fonction pour basculer entre l'affichage du texte et des champs de saisie éditables
    // Importez le composant Popup
    import Popup from "../../components/Popup.svelte";
  
    // Vos fonctions et variables existantes
    let name = "Ramy";
    let isEditing = false;
    let newName = name;
    ...
    function editInfo(fieldName) {
      ...
      const editButton = document.getElementById(`edit-${fieldName}`);
      ...
      inputField.addEventListener("keydown", (event) => {
        if (event.key === "Enter") {
          infoText.textContent = inputField.value;
          inputField.style.display = "none";
          infoText.style.display = "inline-block";
          editButton.style.display = "inline-block";
        }
  ...
  <main>
      ...
      <!-- Popup -->
      <Popup title="Edit Name" isOpen={isEditing} onClose={() => (isEditing = false)}>
      ...
      <div class="Profile-background bg-primary">
        ...
        <div class="ProfileDisplay card-shadow-gray">
          ...
          <input
            ...
            bind:value={newName}
            class="form-control"
            style="width: 80%; display: {isEditing ? 'inline-block' : 'none'};"
          />
          <button
            id="edit-name"
            class="ButtonEdit btn border-0 bg-primary text-white"
            on:click={openPopup}>edit</button
          >
              </div>
            </div>
            <div class="ProfileInfo fs-5">
              ...
              <div class="ProfileEditInfo">
                ...
                <button class="ButtonEdit btn border-0 bg-primary text-white"
                  >edit</button
                >
              </div>
            </div>
